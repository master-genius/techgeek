...
function show_select_media_list(li) {
    ...
    for(var i=0; i< li.length ;i++) {
        imgurl = `${_sysv.image_host}/${li[i].media_name[0]}/${li[i].media_name}`;
        html += `
            <div class="float-block-image">
                <div onclick="media_picked('${imgurl}', ${li[i].id})" style="background-image:url('${imgurl}');background-position:center;background-repeat:no-repeat;background-size:cover;width:100%;height:12rem;">
                </div>
            </div>
        `;
    }
